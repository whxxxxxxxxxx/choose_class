...
func makeFromWithStruct[T any](c *T) string {
...
func (c *DropClassReq) MakeForm() string {
	return makeFromWithStruct(c)
}

func fillStructWithMap[T any](field map[string]string, alias map[string]string) *T {
	var req T
	...
	for k := 0; k < t.NumField(); k++ {
		...
		if alias != nil && alias[tag] != "" {
			v.Field(k).SetString(field[alias[tag]])
		} else {
			v.Field(k).SetString(field[tag])
		}
	}
	return &req
}
...
func MakeGetChosenClassReq(field map[string]string) *GetChosenCourseReq {
	return fillStructWithMap[GetChosenCourseReq](field, map[string]string{
		"jg_id": "jg_id_1",
	})
}

func MakeFindClassReq(field map[string]string) *FindClassReq { //获取基本参数
	req := fillStructWithMap[FindClassReq](field, map[string]string{
		"kklxdm": "firstKklxdm",
		"jg_id":  "jg_id_1",
	})
...
func MakeChooseClassPrvReq(field map[string]string) *ChooseClassPrvReq {
	req := fillStructWithMap[ChooseClassPrvReq](field, map[string]string{
		"kklxdm":  "firstKklxdm",
		"xkkz_id": "firstXkkzId",
	})
...
func MakeGetClassDetailReq(field map[string]string) *GetCourseDetailReq {
	req := fillStructWithMap[GetCourseDetailReq](field, map[string]string{
		"kklxdm":  "firstKklxdm",
		"xkkz_id": "firstXkkzId",
		"jg_id":   "jg_id_1",
	})
...
func MakeDropClassReq(field map[string]string) *DropClassReq {
	return fillStructWithMap[DropClassReq](field, nil)
}

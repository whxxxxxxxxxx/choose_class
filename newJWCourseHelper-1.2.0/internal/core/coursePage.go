...
func (u *User) DropCourse() ([]string, error) {
	if u.Error() != nil {
		log.Printf("find an err: %v", u.Error())
		return nil, u.Error()
	}
	...
	dropList := u.courses.TmpList
	var success []string
	for i := 0; i < len(dropList) && dropList[i].DetailList != nil; i++ {
		// 跳过选课失败的课程 & 未选课程
		{
			if u.checkInErrList(dropList[i].Jxbmc) || !u.checkChosen(dropList[i].Kch) {
				continue
			}
		}
		...
		dropreq := dto.MakeDropClassReq(u.getField())

		err := u.dropCourse(dropreq)
		if err != nil {
			log.Printf("【err】 退选 %s 时发生错误： %v\n", dropList[i].Jxbmc, err.Error())
			u.config.errTag = append(u.config.errTag, dropList[i].Jxbmc)
			continue
		}
		success = append(success, dropList[i].Jxbmc)

		// remove it
		if i == len(dropList)-1 {
			dropList = dropList[:i]
		} else {
			dropList = append(dropList[:i], dropList[i+1:]...)
		}
		i--

...
func ChooseCourse() {
	...
	for _, user := range c {
		...
		resDrop := res.Copy()
		var t1, t2 []core.Target
		for _, v := range user.Target {
			tg := core.Target{
				Name: v.Name,
				Type: v.Type,
			}
		...
		resDrop.SetTarget(t1)
		// 退个课先
		allDropped := core.Drop(resDrop)
		//res.PrintCourseChosenList()                                   //输出已选课程列表
		res.SetTarget(t2) //输出待选课程列表//继续debug，把config文件对应的结构体数组修改好
		// 立即抢课
		core.Job(res)
		res.SetCorn(cron.New())
		// 选课定时任务
		var cornExpr = fmt.Sprintf("*/%d * * * * *", user.Interval)
		if err := res.GetCorn().AddFunc(cornExpr, func() {
			core.Job(res)
		}); err != nil {
		...
		// 退课定时任务
		if !allDropped {
			if err := resDrop.GetCorn().AddFunc(cornExpr, func() {
				core.Job(res)
			}); err != nil {
				log.Printf("用户 %s 定时任务添加失败: %v", user.User.StaffId, err)
			}
